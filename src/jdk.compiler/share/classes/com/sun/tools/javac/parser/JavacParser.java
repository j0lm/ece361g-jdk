...
public class JavacParser implements Parser {
    ...
    JCExpression literal(Name prefix, int pos) {
        ...
        switch (token.kind) {
            ...
            case LONGLITERAL:
                ...
                break;
            case FLOATLITERAL: {
                String proper = token.radix() == 16 ? ("0x" + token.stringVal()) : token.stringVal();
                Float n;
                try {
                ...
                if (token.kind == LPAREN) {
                    ...
                    if (token.kind == IDENTIFIER) {
                        ...
                        if (mods.annotations.nonEmpty()) {
                            ...
                            List<JCAnnotation> typeAnnos = mods.annotations
                                    .map(decl -> {
     ...
     * | Arguments
     * | "." ( CLASS | THIS | [TypeArguments] SUPER Arguments | NEW [TypeArguments]
     * InnerCreator )
     * ]
    ...
    JCExpression lambdaExpressionOrStatement(boolean hasParens, boolean explicitParams, int pos) {
        List<JCVariableDecl> params = explicitParams ? formalParameters(true, false) : implicitParameters(hasParens);
        if (explicitParams) {
                ...
                if (param.vartype != null &&
                    ...
                    log.error(DiagnosticFlag.SYNTAX, param.pos,
                            Feature.VAR_SYNTAX_IMPLICIT_LAMBDAS.allowedInSource(source)
                                    ...
                                    : Errors.RestrictedTypeNotAllowedHere(restrictedTypeName));
                }
            ...
            if (lambdaClassifier.diagFragment != null) {
                log.error(DiagnosticFlag.SYNTAX, pos,
                        Errors.InvalidLambdaParameterDeclaration(lambdaClassifier.diagFragment));
            }
            for (JCVariableDecl param : params) {
                if (param.vartype != null
        ...
        private void reduce(LambdaParameterKind newKind) {
            if (kind == null) {
                ...
                diagFragment = Feature.VAR_SYNTAX_IMPLICIT_LAMBDAS.allowedInSource(source) || !varIndex
                        ? decisionTable[currentKind.index][newKind.index]
                        : null;
            }
        }
    ...
    JCExpression lambdaExpressionOrStatementRest(List<JCVariableDecl> args, int pos) {
        ...
        return token.kind == LBRACE ? lambdaStatement(args, pos, token.pos) : lambdaExpression(args, pos);
    }
    ...
    JCExpression bracketsSuffix(JCExpression t) {
        if ((mode & EXPR) != 0 && token.kind == DOT) {
            ...
            if (token.pos == endPosTable.errorEndPos) {
                ...
                Tag tag = t.getTag();
                // Type annotations are illegal on class literals. Annotated non array class
                // literals
                // are complained about directly in term3(), Here check for type annotations on
                // dimensions
            ...
            case BOOLEAN:
                if (typeArgs == null) {
                    if (newAnnotations.isEmpty()) {
                        ...
                        return arrayCreatorRest(newpos,
                                toP(F.at(newAnnotations.head.pos).AnnotatedType(newAnnotations, basicType())));
                    }
                }
                break;
            default:
     ...
     * not legal for a Statement) by delegating the parsing to BlockStatement (JLS
     * 14.2).
     * If any illegal declarations are found, they will be wrapped in an erroneous
     * tree,
                ...
                if (token.name() == names.yield && allowYieldStatement) {
                    ...
                    if (isNonSealedClassStart(true)) {
                        ...
                        return List.of(classOrRecordOrInterfaceOrEnumDeclaration(modifiersOpt(),
                                token.comment(CommentStyle.JAVADOC)));
    ...
    private List<JCStatement> localVariableDeclarations(JCModifiers mods, JCExpression type) {
        ListBuffer<JCStatement> stats = variableDeclarators(mods, type, new ListBuffer<>(), true);
     ...
     * TODO: the "FormalParameter" is not correct, it uses the special "catchTypes"
     * rule below.
     */
        ...
        while (true) {
            ...
            switch (token) {
                ...
                case ASSERT, ENUM, IDENTIFIER, UNDERSCORE:
                    ...
                    break;
                case DOT, QUES, EXTENDS, SUPER, COMMA:
                    break;
                case LT:
                    depth++;
                    break;
    ...
    Name restrictedTypeName(JCExpression e, boolean shouldWarn) {
        switch (e.getTag()) {
            case IDENT:
                return restrictedTypeNameStartingAtSource(((JCIdent) e).name, e.pos, shouldWarn) != null
                        ? ((JCIdent) e).name
                        : null;
            case TYPEARRAY:
        ...
        for (JCTree def : defs) {
            if (def.hasTag(METHODDEF)) {
                ...
                if (methDef.name == names.init && methDef.params.isEmpty()
                        && (methDef.mods.flags & Flags.COMPACT_RECORD_CONSTRUCTOR) != 0) {
                    ...
                    for (JCVariableDecl param : headerFields) {
                        tmpParams.add(F.at(param)
                                ...
                                .VarDef(F.Modifiers(
                                        Flags.PARAMETER | Flags.GENERATED_MEMBER | param.mods.flags & Flags.VARARGS,
    ...
    protected JCClassDecl enumDeclaration(JCModifiers mods, Comment dc) {
        ...
        JCClassDecl result = toP(F.at(pos).ClassDef(mods, name, List.nil(),
    ...
    private EnumeratorEstimate estimateEnumeratorOrMember(Name enumName) {
        // if we are seeing a record declaration inside of an enum we want the same
        // error message as expected for a
    ...
    /**
     * EnumeratorDeclaration = AnnotationsOpt [TypeArguments] IDENTIFIER [ Arguments
     * ] [ "{" ClassBody "}" ]
     */
     ...
     *
     */
    protected List<JCTree> classOrInterfaceOrRecordBodyDeclaration(Name className, boolean isInterface,
            boolean isRecord) {
        if (token.kind == SEMI) {
            nextToken();
            return List.nil();
                ...
                if ((token.kind == LPAREN && !isInterface ||
                    ...
                    if (token.kind == LPAREN) {
                        ...
                        if (!isRecord || (isRecord && (mods.flags & Flags.STATIC) != 0)) {
                            List<JCTree> defs = variableDeclaratorsRest(pos, mods, type, name, isInterface, dc,
                        ...
                        if (isVoid || typarams.nonEmpty()) {
                            JCMethodDecl m = toP(F.at(pos).MethodDef(mods, name, type, typarams,
    ...
    protected JCTree methodDeclaratorRest(int pos,
        ...
        try {
            ...
            JCMethodDecl result = toP(F.at(pos).MethodDef(mods, name, type, typarams,
